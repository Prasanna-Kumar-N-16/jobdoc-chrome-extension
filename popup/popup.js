  ...
  if (lastRawReport) {
    ...
    if (banner) {
      ...
      document.getElementById("sessionInfo").textContent =
        "📂 Restored — " + new Date(session.savedAt).toLocaleString();
    }
    document.getElementById("scoreSub").textContent = "Restored from last session";
    switchTab("results");
  }
}
...
const CORS_CMDS = {
  mac: {
    ...
    note: "Paste into Terminal. Sets the env var permanently. If Ollama isn't in /Applications: OLLAMA_ORIGINS=\"*\" ollama serve"
  },
  win: {
    cmd: `setx OLLAMA_ORIGINS "*"\ntaskkill /IM "ollama app.exe" /F\nstart "" "%LOCALAPPDATA%\\Programs\\Ollama\\ollama app.exe"`,
    note: "Paste into Command Prompt (run as Admin). setx makes it permanent."
  },
  linux: {
    cmd: `echo 'export OLLAMA_ORIGINS="*"' >> ~/.bashrc\nsource ~/.bashrc\npkill -f "ollama serve"\nOLLAMA_ORIGINS="*" ollama serve &`,
    note: "Paste into Terminal. Using systemd? sudo systemctl edit ollama — add Environment=OLLAMA_ORIGINS=* then sudo systemctl restart ollama"
  }
};

function showCors() { document.getElementById("corsBox").style.display = "block"; }
function hideCors() { document.getElementById("corsBox").style.display = "none"; }
function showCmd(os) {
...
async function recheckCors() {
  try {
    ...
    if (r?.online) {
      ...
      document.getElementById("cmdNote").textContent = "⚠ Still not reachable. Make sure Ollama restarted.";
    }
  } catch (e) {
...
async function analyze(jdOverride) {
  const jd = jdOverride || document.getElementById("jdInput").value.trim();
  if (!jd || jd.length < 50) {
  ...
  try {
    ...
    if (!status?.online) { hideProgress(); showCors(); return; }

    setProgressMsg("Analyzing with Ollama — this takes 30–120s…");
    document.getElementById("streamPreview").style.display = "block";

    const result = await bg({ action: "fullAnalysis", jd, profile: settings });

    if (!result.success) {
      hideProgress();
      if (result.error?.includes("403") || result.error?.includes("CORS")) showCors();
      else showErr(result.error);
      return;
    }
...
// ── RETRY ─────────────────────────────────────────────────────────────────────
// Re-runs the full analysis with the same JD — produces a fresh report.

async function retryAnalysis() {
  const jd = lastJD || document.getElementById("jdInput").value.trim();
  if (!jd || jd.length < 50) {
    alert("No previous job description found. Please paste one in the Analyze tab first.");
    switchTab("jd");
    return;
  }
  // Reset score bar so it animates again
  document.getElementById("scoreBar").style.width = "0";
  document.getElementById("atsScore").textContent = "—";
  document.getElementById("scoreTitle").textContent = "Retrying…";
  document.getElementById("scoreSub").textContent = "Running a fresh analysis…";
...
function showProgress(msg) {
  ...
  document.getElementById("jdInput").readOnly = true;
  ...
  document.getElementById("btnCancelAnalysis").disabled = false;
}

function hideProgress() {
  const btn = document.getElementById("btnAnalyze");
  btn.disabled = false;
  btn.textContent = btn.dataset.originalLabel || "✦ Analyze & Generate Resume";
  document.getElementById("jdInput").readOnly = false;
  document.getElementById("progressBox").style.display = "none";
}

function setProgressMsg(msg) { document.getElementById("progressMsg").textContent = msg; }
...
chrome.runtime.onMessage.addListener((msg) => {
  if (msg.action === "streamToken") {
    ...
    setProgressMsg("Generating… " + el.textContent.length + " chars");
  }
});
...
function renderReport(raw, atsScore) {
  ...
  if (score > 0) {
    ...
    document.getElementById("scoreTitle").textContent = score >= 75 ? "Strong Match" : score >= 50 ? "Moderate Match" : "Needs Work";
    document.getElementById("scoreSub").textContent   = "ATS compatibility — review improvements below";
    setTimeout(() => { document.getElementById("scoreBar").style.width = score + "%"; }, 100);
  }
  document.getElementById("reportContent").innerHTML = markdownToHTML(raw);
}
...
<title>ATS Report — ${s.firstName} ${s.lastName}</title>
<style>body{font-family:Georgia,serif;max-width:800px;margin:40px auto;padding:20px;color:#1a1a1a;line-height:1.6;}h1{font-size:22pt;border-bottom:2px solid #333;padding-bottom:8px;}h2{font-size:14pt;color:#1e40af;margin-top:28px;border-bottom:1px solid #cbd5e1;padding-bottom:4px;}h3{font-size:12pt;color:#374151;}table{width:100%;border-collapse:collapse;margin:12px 0;}th{background:#f1f5f9;font-weight:700;padding:8px 10px;border:1px solid #cbd5e1;text-align:left;}td{padding:8px 10px;border:1px solid #cbd5e1;vertical-align:top;}tr:nth-child(even) td{background:#f8fafc;}ul{padding-left:18px;}li{margin-bottom:4px;}@media print{body{margin:0;}}</style>
</head><body>
...
${markdownToHTML(lastRawReport)}
  ...
  try { await bg({ action:"download", content:html, filename:`ATS_Report_${s.firstName}_${s.lastName}.html`, mime:"text/html" }); }
  catch(e) { if (!isPortClosedError(e)) showErr(e.message); }
}

async function downloadResume() {
  if (!lastResumeHTML) return;
  const s = await chrome.storage.local.get(["firstName","lastName"]);
  try { await bg({ action:"download", content:lastResumeHTML, filename:`Resume_${s.firstName}_${s.lastName}.html`, mime:"text/html" }); }
  catch(e) { if (!isPortClosedError(e)) showErr(e.message); }
}

function previewResume() {
  if (!lastResumeHTML) { alert("Generate a report first to create the ATS resume."); return; }
  const blob = new Blob([lastResumeHTML], { type:"text/html" });
  chrome.tabs.create({ url: URL.createObjectURL(blob) });
}

async function autofill() {
  const s = await chrome.storage.local.get(["firstName","lastName","email","phone","location","linkedin","website","github","portfolio"]);
  s.coverLetter = lastCoverLetter;
  if (!s.website && s.portfolio) s.website = s.portfolio;
  const [tab] = await chrome.tabs.query({ active:true, currentWindow:true });
  try {
    const r = await bg({ action:"autofillTab", tabId:tab.id, profile:s });
    document.getElementById("scoreSub").textContent = r?.success
      ? `✓ Filled ${r.result?.filled||0} fields`
      : `⚠ ${r?.error||"Partial fill"}`;
  } catch(e) {
...
async function loadSettings() {
  ...
  setValue("sFirst", d.firstName||"");  setValue("sLast", d.lastName||"");
  setValue("sEmail", d.email||"");      setValue("sPhone", d.phone||"");
  setValue("sLoc",   d.location||"");   setValue("sLinkedin", d.linkedin||"");
  setValue("sGithub",d.github||"");     setValue("sPortfolio", d.portfolio||d.website||"");
  setValue("sUrl",   d.ollamaUrl||"http://localhost:11434");
  try { setValue("sResume", d.resumeData ? JSON.stringify(JSON.parse(d.resumeData),null,2) : ""); }
  ...
  if (d.ollamaModel) {
    ...
    else sel.appendChild(new Option(d.ollamaModel, d.ollamaModel, true, true));
  }
}

async function saveSettings() {
  const resumeRaw = document.getElementById("sResume").value.trim();
  if (resumeRaw) { try { JSON.parse(resumeRaw); } catch { showToast("toastErr","Resume JSON is invalid."); return; } }
...
async function resetToNewAnalysis() {
  await clearSession();
  lastRawReport = ""; lastResumeHTML = ""; lastCoverLetter = ""; lastJD = "";
...
function setupListeners() {
  ...
  // ── RETRY BUTTON ──────────────────────────────────────────────────────────
  document.getElementById("btnRetry").addEventListener("click", retryAnalysis);
  ...
  document.getElementById("btnLoadModels").addEventListener("click", async () => {
    ...
    } catch(e) { if (!isPortClosedError(e)) showToast("toastErr", e?.message||"Failed"); }
  });
...
async function cancelAnalysis() {
  ...
  try {
    ...
    document.getElementById("scoreSub").textContent =
      r?.cancelled ? "Generation cancelled." : "No active generation to cancel.";
  } catch(e) {
    hideProgress();
    if (!isPortClosedError(e)) showErr(e.message);
...
function bg(msg) {
  return new Promise((res,rej) => {
    chrome.runtime.sendMessage(msg, r => {
      ...
      if (err) { const e = new Error(err.message); if (err.message?.includes("message port closed")) e.code="PORT_CLOSED"; rej(e); }
      else res(r);
    });
  });
}
...
function showErr(msg) { alert("Error: " + (msg||"Unknown error")); }
